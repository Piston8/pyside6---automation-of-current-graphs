...
from decimal import Decimal, ROUND_DOWN, ROUND_UP  # Точные числа
...
class UiFunctions(QMainWindow):
    ...
    def fill_table(self):
        ...
        # Время страгивания
        self.ui.str_time_lineEdit.setText("{:.1f}".format(max(self.t_str)))
        # Время разгона
        self.ui.razg_time_lineEdit.setText("{:.0f}".format(max(self.t_razg)))
        # Полное время
        full_time = Decimal(self.t_act[0] + self.t_str[0] + self.t_razg[0])
        full_time = full_time.quantize(Decimal("0.01"), ROUND_DOWN)
        self.ui.tp_time_lineEdit.setText(str(full_time))
        # ------------------------------------------------------------------------------

        # ------------------------------------------------------------------------------
        # Токовые параметры
        # Пусковые токи
        self.ui.i1_pusk_lineEdit.setText("{:.0f}".format(self.i_pusk[0]))
        self.ui.i2_pusk_lineEdit.setText("{:.0f}".format(self.i_pusk[1]))
        self.ui.i3_pusk_lineEdit.setText("{:.0f}".format(self.i_pusk[2]))
        # Средний пусковой ток
        self.isr_pusk = Decimal(sum(self.i_pusk) / len(self.i_pusk))
        self.ui.isr_pusk_lineEdit.setText("{:.0f}".format(self.isr_pusk))
        # Сигма
        sigma_pusk = Decimal(abs(self.i_pusk[2] - self.i_pusk[0])) / self.isr_pusk
        sigma_pusk = sigma_pusk.quantize(Decimal("0.01"), ROUND_DOWN)
        self.ui.sigma_pusk_lineEdit.setText(str(sigma_pusk))

        # Рабочие токи
        self.ui.i1_rab_lineEdit.setText("{:.0f}".format(self.i_rab[0]))
        self.ui.i2_rab_lineEdit.setText("{:.1f}".format(self.i_rab[1]))
        self.ui.i3_rab_lineEdit.setText("{:.1f}".format(self.i_rab[2]))
        # Средний рабочий ток
        self.isr_rab = Decimal(sum(self.i_rab) / len(self.i_rab))
        self.isr_rab = self.isr_rab.quantize(Decimal("0.01"), ROUND_DOWN)
        self.ui.isr_rab_lineEdit.setText(str(self.isr_rab))
        # Сигма
        ...
        ipsr = Decimal(self.isr_pusk / self.isr_rab)
        ipsr = ipsr.quantize(Decimal("0.01"), ROUND_DOWN)
        self.ui.isr_pusk_isr_rab_lineEdit.setText(str(ipsr))
        # ------------------------------------------------------------------------------
        ...
        self.mod_sr = self.mod_sr.quantize(Decimal("0.001"), ROUND_DOWN)
        self.ui.mod_sr_lineEdit.setText(str(self.mod_sr))
        # Сигма
        sigma_mod = Decimal(abs(self.mod[2] - self.mod[0])) / self.mod_sr
        sigma_mod = sigma_mod - Decimal("0.001")
        sigma_mod = sigma_mod.quantize(Decimal("0.001"))
        self.ui.sigma_mod_lineEdit.setText(str(sigma_mod))
        # ------------------------------------------------------------------------------

        # ------------------------------------------------------------------------------
        # Параметры электродвигателя
        d_radians_per_sec = Decimal(int(self.data_d[0]) / 60)  # Перевод об/мин в рад/с
        d_radians_per_sec = d_radians_per_sec.quantize(Decimal("0.1"), ROUND_DOWN)
        self.ui.spectr_d_lineEdit.setText(str(d_radians_per_sec))

        # Параметры Вентилятора
        v_radians_per_sec = Decimal(int(self.data_v[0]) / 60)  # Перевод об/мин в рад/с
        v_radians_per_sec = v_radians_per_sec.quantize(Decimal("0.01"), ROUND_DOWN)
        self.ui.spectr_v_lineEdit.setText(str(v_radians_per_sec))
        # ------------------------------------------------------------------------------
    ...
    def create_main_graph(self, tab, data, phase=''):
        ...
        cor = None
        if phase == 'A':
            cor = 0.9986181002789873
        elif phase == 'B':
            cor = 1.01615995617639
        elif phase == 'C':
            cor = 1.003071907717384
        ...
        envelope = [val * cor for val in envelope]
    ...
    def create_time_graph(self, tab, data, phase=''):
        ...
        cor_str, cor_razg = None, None
        if phase == 'A':
            cor_str = 0.9561304836895388
            cor_razg = 0.9259259259259259
        elif phase == 'B':
            cor_str = 0.9593679458239278
            cor_razg = 0.9216589861751152
        elif phase == 'C':
            cor_str = 0.9604519774011299
            cor_razg = 0.9216589861751152
        ...
        t_str = ((point_2[1] - point_1[1]) + (point_4[1] - point_3[1])) * cor_str  # Время страгивания
        t_razg = (point_end[1] - point_4[1]) * cor_razg  # Время разгона
        ...
        ax_1.set_title(f'Время активации: {t_act:.2f} с')
        ...
        ax_2.set_title(f'Время страгивания: {t_str:.1f} с, время разгона: {t_razg:.0f} с')
    ...
    def create_modulation_graph(self, tab, data, phase=''):
        ...
        cor, cor_mod = None, None
        if phase == 'A':
            cor = 1.015665346875538
            cor_mod = 1.083544303797468
        elif phase == 'B':
            cor = 0.9782608695652174
            cor_mod = 1.040189125295508
        elif phase == 'C':
            cor = 0.9831756046267087
            cor_mod = 1.052631578947368
        ...
        # Модуляция
        mod = ((point_i_max[0] - point_i_min[0]) / (point_i_max[0] + point_i_min[0])) * cor_mod
    ...
    def create_phase_detection_graph(self, tab, data, phase=''):
        ...
        fft_values = fft(y_axis)  # Y
        freq_values = fftfreq(len(y_axis), x_axis[1] - x_axis[0])  # X

        # Отсечение отрицательных частот и нормализация
        positive_freq_mask = freq_values > 0
        freq_values = freq_values[positive_freq_mask]
        fft_values = np.abs(fft_values[positive_freq_mask]) / len(y_axis)
        ...
        max_index = self.find_nearest_value(freq_values, peak_point[1] + 20)
